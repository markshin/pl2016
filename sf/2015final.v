...
Definition sort_two : com :=
 IFB (BLe (AId X) (AId Y)) THEN (X ::= (AId X) ;; Y ::= (AId Y))
 ELSE ( Z ::= (AId X) ;; X ::= (AId Y) ;; Y ::= (AId Z))
 FI
.
...
auto. 
  ...
  (* If you defined [sort_two] correctly, the following code should work.*)
  (*
  intros.
  repeat match goal with [H: ceval _ _ _ |- _] => rename H into _X; dependent destruction _X end. 
  auto.
...
Proof.
    ...
    simpl.
    intros. split.
    intros. split. SearchAbout ble_nat. apply ble_nat_true in H0.
    assumption.
    left. assumption.
    intros.
    split.
    -  destruct H. rewrite H in H0. rewrite H1 in H0. 
    rewrite H. rewrite H1.
    
    SearchAbout ble_nat.
    SearchAbout ble_nat.
    SearchAbout "<=".
    apply ble_nat_false in H0.
    apply not_le in H0.
    apply gt_le_S in H0.
    SearchAbout "ble_nat".
    SearchAbout "<=". 
    ...
    -
    right. destruct H. split; assumption.
    Qed.
(*-- Check --*)
...
Proof.
  unfold sum_com. 
  intros.
  apply hoare_seq with (fun st => st N = n /\ st SUM = 0).
  eapply hoare_consequence_post. 
    ...
    admit. (*split. Print Nat.sub_0_r. apply negb_true_iff in H0. 
    ...
    simpl in H0. SearchAbout negb. 
        apply negb_false_iff in H0. 
        SearchAbout beq_nat. 
            apply beq_nat_true in H0. unfold sum. rewrite H1.     
    simpl.
        admit.
        -  admit.   
        Qed. 
...
Proof.
 ...
 unfold sum2_com.
 apply hoare_seq with (fun st => st N = n /\ st SUM = 0).
 apply hoare_seq with (fun st => st N = n /\ st SUM = 0 /\ st I = 0). 
 eapply hoare_consequence_post.
 apply hoare_while. 
...
Definition tmmultpair : tm :=
  (tfix (tabs Y (TArrow (TProd TNat TNat) TNat)
        (tabs X (TProd TNat TNat) (tmult (tfst (tvar X)) (tsnd (tvar X)))))).
